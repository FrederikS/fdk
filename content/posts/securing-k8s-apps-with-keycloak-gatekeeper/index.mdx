...
* Create a new realm
  * To not run into trouble with login later on since I have not set up proper certificates for tls I'll set a insecure "Frontend URL" to
  `http://keycloak.local/auth`
* Add a client with protocol `openid-connect` and access-type: `confidential`
  * Add a Redirect-URL: `http://httpbin.local/oauth/callback`
* Goto to the "Client Scopes" menu
  * Add a new client scope and enter its settings afterwards enter Mappers tab and create new protocol with mapper type audience
  * Select your client in "Included Client Audience"
  * Set "Add to access token" to "On"
* In client settings go to client scopes
  * Add your created client scope containing the audience mapper
* Add a user
