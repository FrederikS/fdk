...
date: 2020-04-13
description: For java development its often necessary to be able to switch between different jdk versions. One way of handle this is to use SDKMAN and this is my machines' setup.
tags:
  - java
  - sdkman
  - archlinux
...
For java development its often necessary to be able to switch between different jdk versions.
One way of handle this is to use SDKMAN and this is my machines' setup.

### Installation

There is one AUR package available called `sdkman` but its seems to be for an unoffical client of sdkmal.
To use the official one you should follow the instructions from [here](https://sdkman.io/install). It's pretty fast and easy.
Basically you install it via curl from the commandline.
...
So far so good. We are ready to use sdkman. To install a java version we check which of them are available first.
...
To install one of one of the candidates we are using the identifier from the last column of the output table.
...
As long as you only working from the commandline in your shell everything is working fine.
But in case you want to work with an IDE like **vscode-java** and you are not starting it from the commandline
the current java version is not picked up properly.
Why not and how does it work? There are several ways for **vscode-java** to define the jdk path that should be use.
...
#### What is the problem sdkman already exports `JAVA_HOME`?
...
Normally you would define global environment variable somewhere in `/etc/enviroment` or `/etc/profile`.
But since sdkman runs on our user level it is a bad idea to put something for user level into system scope and other users would get trouble.

#### Fixing env variables

What we want is to define it global but only for our user.
One way to do it, is to use the `systemd` daemon and put or enviroment variable to a conf file into our `~/.config/environment.d/` directory.
