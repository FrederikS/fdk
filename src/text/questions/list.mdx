...
`LinkedList` has a higher complexity of O(n) for random access via
...
at a specific index are on average faster for `LinkedList`.

Therefore a `LinkedList` is preferable when you rather have prepends, head/tail
